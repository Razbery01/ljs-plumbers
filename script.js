// LJS Plumbers & Drainlayers — interactions
...
function ljsSubmit(e) {
  ...
  const text =
    `Hi LJS Plumbers, I need help.%0A%0A` +
    `Name: ${name}%0A` +
    `Phone: ${phone}%0A` +
    `Suburb: ${suburb}%0A%0A` +
    `${msg}`;
  window.open(`https://wa.me/27638742223?text=${text}`, '_blank', 'noopener');
  return false;
}
window.ljsSubmit = ljsSubmit;
